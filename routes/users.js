...
router.get('/', async function(req, res, next) {
 ...
 const auth = await request(`${gateway}/api/user/access/current`,'GET',{}, {
  ...
  // 3.获取公告列表
  const list = await request('http://localhost:7001/api/user?isPaging=false','GET',{}, {
    'Authorization': `Bearer ${token}`
  })
...
if(auth.code != 0) {
  ...
  const createRes = await request(`http://localhost:7001/api/user`,'POST', params, {
     'Authorization': `Bearer ${token}`
  })
...
if(auth.code != 0) {
 ...
 const delRes = await request(`http://localhost:7001/api/user/${_id}`,'DELETE', {}, {
  'Authorization': `Bearer ${token}`
})
...
if(auth.code != 0) {
  ...
  const updateRes = await request(`http://localhost:7001/api/user/${params.id}`,'PUT', params, {
     'Authorization': `Bearer ${token}`
  })
